 ...
 */
export class SearchSubListControl extends Component {
	constructor() {
		super( ...arguments );
		...
		return list.map( ( item ) => (
			...
			:
				...
				<Fragment key={ item.id }>
					...
					{ item.children && item.children.length > 0 ? <div onClick={ this.onSelect( item ) } className="holder-container woocommerce-search-list__list woocommerce-search-list__item"><span style={{minWidth: "35px"}}></span>{item.children.length} options to choose from...</div> : '' }
				</Fragment>
...
SearchSubListControl.propTypes = {
	/**
